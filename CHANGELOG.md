...
- App now reacts to CloudKit / KVS remote changes by reloading the in-memory entity caches, re-sharing widget data, rescheduling notifications, and refreshing badges. Without this, sync would happen at the data layer but the UI would look stale until restart. The remote-change observer is only registered when iCloud sync is on — `.NSPersistentStoreRemoteChange` fires for local saves too, and reloading on every save would wipe transient state (e.g. a freshly inserted Pill's `isCreated = false` would get clobbered before navigation).
